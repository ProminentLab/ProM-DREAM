	...
	private void estimate() {
		...
		for (Place key : activations.keySet()) {
			...
			for (Tuple<?, ?> tpl : actLst) {
				if (currTpl != tpl.x.intValue()) {
					currTpl = tpl.x.intValue();
					delta = -1.0;
					prevTime = tpl.y.longValue();
				} else {
					delta = new Long(tpl.y.longValue() - prevTime).doubleValue();
				}
				prevTime = tpl.y.longValue();
		...
		for (Place key : deltaActivations.keySet()) {
			...
			for (Tuple<?, ?> t : values) {
				if (t.y.doubleValue() != -1.0) {
					vec.add(t.y.doubleValue());
				}
			}
