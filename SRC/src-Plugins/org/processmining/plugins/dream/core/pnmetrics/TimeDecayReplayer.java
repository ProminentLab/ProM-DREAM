...
public class TimeDecayReplayer {
	...
	public void createDataset(String suffix){
		...
		try {
			File fouttrain = new File("C:\\Users\\Julian\\Documents\\GitHub\\ProM\\decaydata\\" + this.name + "_" + suffix + ".csv");
	...
	public TimedStateSamples createTimedStateSamples() {	
		int len = this.decayVecPreds.get(0).vector.length;
		int resources = 0;
		List<String> data = new ArrayList<String>();	
		for (DecayVectorPrediction pred : decayVecPreds) {
			data.add(pred.toString());
			if (pred.resources.size() > resources){
				resources = pred.resources.size();
			}
		}
		
		TimedStateSamples tss = new TimedStateSamples(len, resources);	
